...
For efficient data processing, the data is stored in a planar (also known as
columnar or SoA) form.
...
The `BlockVector` constructors verifies that the offset vector is well-formed.
...
When indexed by a vector of indexes, a instance of `BlockVector` is returned.
...
used to represent self-referential data structures.
