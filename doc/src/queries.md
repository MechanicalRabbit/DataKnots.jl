...
`tuple_lift(>)` that transforms a `TupleVector` instance with two integer
...
`block_lift(length)` that transforms a block vector to an integer vector
...
`filler()` generates a primitive operation that maps any input vector to
...
Similarly, the output of `block_filler()` is a block vector filled with the given
block.
...
A variant of `block_filler()` called `null_filler()` outputs a block vector with
empty blocks.
