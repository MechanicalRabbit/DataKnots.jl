...

{
    ...
    "title": "Query Combinators",
    "category": "section",
    "text": "There are query operations which cannot be automatically lifted from Julia functions. These require knowledge DataKnot storage, flows, context, and other internal details. We\'ve met two of them, Lift itself and query composition (>>). There are many others involving aggregation, filtering, paging and others."
},

{
    ...
    "title": "Aggregate Queries",
    "category": "section",
    "text": "So far queries have been elementwise; that is, for each input element, they produce zero or more output elements. Consider now the Count primitive has a singular output for its entire input.void[OneTo(3) >> Count]\n#=>\n│ It │\n┼────┼\n│  3 │\n=#We call these kinds of queries aggregates. While aggregate queries may reflect the elements of their input, the output is relative to the input as whole. Consider OneTo(3) >> OneTo(It).void[OneTo(3) >> OneTo(It)]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  1 │\n3 │  2 │\n4 │  1 │\n5 │  2 │\n6 │  3 │\n=#By simply appending >> Sum we could aggregate the input.void[OneTo(3) >> OneTo(It) >> Sum]\n#=>\n│ It │\n┼────┼\n│ 10 │\n=#What if we wanted to produce sums by the outer query, OneTo(3)? Since query composition (>>) is associative, adding parenthesis around OneTo(It) >> Sum will not change the result.void[OneTo(3) >> (OneTo(It) >> Sum)]\n#=>\n│ It │\n┼────┼\n│ 10 │\n=#What we need is the Each combinator, which acts as a barrier for an aggregate query. For each of its input elements, Each evaluates its argument and collects the outputs.void[OneTo(3) >> Each(OneTo(It) >> Sum)]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  3 │\n3 │  6 │\n=#Following is an equivalent query, using the Sum combinator. While Sum(X) performs a numerical aggregation, it is not an aggregate query since it treats its input elementwise.void[OneTo(3) >> Sum(OneTo(It))]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  3 │\n3 │  6 │\n=#Julia functions taking a vector argument, such as mean, can be easily lifted. DataKnots automatically converts a plural query parameter into the vector argument.using Statistics\nMean(X) = mean.(X)\nvoid[Mean(OneTo(3) >> Sum(OneTo(It)))]\n#=>\n│ It      │\n┼─────────┼\n│ 3.33333 │\n=#To use Mean as a query primitive, we use Then to build a query that aggregates from its input. Next, we register this query so it is chosen when Mean is implicitly converted to a query.DataKnots.Lift(::typeof(Mean)) = DataKnots.Then(Mean)Once these are done, one could take an average of sums as follows:void[Lift(1:3) >> Sum(OneTo(It)) >> Mean]\n#=>\n│ It      │\n┼─────────┼\n│ 3.33333 │\n=#In DataKnots, summary operations are naturally expressed as aggregate query primitives or as query combinators. Moreover, custom aggregates can be constructed from native Julia functions and lifted into the query algebra."
},
...

{
    ...
    "title": "Paging Data",
    "category": "section",
    "text": "Like Filter, the Take and Drop combinators can be used to choose elements from its input stream: Drop is used to skip over input, while Take ignores output past a particular point.void[OneTo(9) >> Drop(3) >> Take(3)]\n#=>\n  │ It │\n──┼────┼\n1 │  4 │\n2 │  5 │\n3 │  6 │\n=#Unlike Filter, the argument to Take is evaluated once, in the context of the input\'s origin. In this next example, Take is performed three times, and each time, It refers to the integer elements of the outer loop, OneTo.void[OneTo(3) >> Each(Lift(\'a\':\'c\') >> Take(It))]\n#=>\n  │ It │\n──┼────┼\n1 │ a  │\n2 │ a  │\n3 │ b  │\n4 │ a  │\n5 │ b  │\n6 │ c  │\n=#What if we want to grab the 1st half of an input stream? Let\'s define FirstHalf as a combinator that builds a query returning the first half of an input stream.FirstHalf(X) = Each(X >> Take(Count(X) .÷ 2))\nvoid[FirstHalf(OneTo(6))]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  2 │\n3 │  3 │\n=#Using Then, this combinator could be used as a query primitive.DataKnots.Lift(::typeof(FirstHalf)) = DataKnots.Then(FirstHalf)\n\nvoid[OneTo(6) >> FirstHalf]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  2 │\n3 │  3 │\n=#Paging operations highlight a diversity of query operations. When a query processes its input an element at a time, we call it elementwise; else, it is aggregate. Further, when the argument of a combinator uses the input\'s origin, we call it origin-aware.Query Elementwise Origin-Aware\nCount No No\nTake(N) No Yes\nCount(X) Yes No\nFilter(P) Yes NoSince both Filter and Count combinators are elementwise and not origin-aware, they behave in quite similar ways. Conversely, the Take combinator behaves more like the Count aggregate, with its argument being origin-aware."
},
