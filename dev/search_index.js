...

{
    ...
    "title": "Getting Started",
    "category": "section",
    "text": "DataKnots is currently usable for contributors who wish to help grow the ecosystem. However, DataKnots is not yet usable for general audiences: with v0.1, there are no data source adapters and we lack important operators, such as Sort and Group. Many of these deficiencies were prototyped and subsequent releases will address these feature gaps incrementally.This is a Julia language library; hence, the DataKnot syntax is necessarily in the Julia language. We\'ve taken pride to provide a usable notation even without using macros. In a future release we will likely include a DataKnot macro syntax as well. Such a macro syntax would be compatible with this language integrated approach."
},
...

{
    ...
    "title": "Navigation",
    "category": "section",
    "text": "To list all the department names we write It.department.name. In this pipeline, It means \"use the current input\". The dotted notation lets one navigate via hierarchy.run(ChicagoData, It.department.name)\n#=>\n  │ name   │\n──┼────────┼\n1 │ POLICE │\n2 │ FIRE   │\n=#Navigation context matters. For example, employee tuples are not directly accessible from the root of the dataset provided. When a label can\'t be found, an appropriate error message is displayed.run(ChicagoData, It.employee)\n#-> ERROR: cannot find employee ⋮Instead, employee tuples can be accessed by navigating though department tuples.run(ChicagoData, It.department.employee)\n#=>\n  │ employee                            │\n  │ name       position          salary │\n──┼─────────────────────────────────────┼\n1 │ JEFFERY A  SERGEANT          101442 │\n2 │ NANCY A    POLICE OFFICER     80016 │\n3 │ DANIEL A   FIRE FIGHTER-EMT   95484 │\n=#Notice that nested lists traversed during navigation are flattened into a single output."
},
...

{
    ...
    "category": "section",
    "text": "Suppose we want a parameterized pipeline that could take other pipelines as arguments. Let\'s define EmployeesOver() to return employee records that have salary greater than a given amount.EmployeesOver(X) = (\n    It.department\n    >> It.employee\n    >> Filter(It.salary .> X))\n\nrun(ChicagoData, EmployeesOver(100000))\n#=>\n  │ employee                    │\n  │ name       position  salary │\n──┼─────────────────────────────┼\n1 │ JEFFERY A  SERGEANT  101442 │\n=#Let\'s list employees having greater than average salary. To start, we must first compute the average salary.AvgSalary = mean.(It.department.employee.salary)\n\nrun(ChicagoData, AvgSalary)\n#=>\n│ It      │\n┼─────────┼\n│ 92314.0 │\n=#We could use this knot value as a parameter to a subsequent run of EmployeesOver(). This works, but is not elegant.run(ChicagoData,\n    EmployeesOver(It.AMT),\n    AMT=run(ChicagoData, AvgSalary))\n#=>\n  │ employee                            │\n  │ name       position          salary │\n──┼─────────────────────────────────────┼\n1 │ JEFFERY A  SERGEANT          101442 │\n2 │ DANIEL A   FIRE FIGHTER-EMT   95484 │\n=#However, if we try to combine these two pipelines directly, we get a naming error.run(ChicagoData, EmployeesOver(AvgSalary))\n#-> ERROR: cannot find department ⋮By looking at the definition, we can see the problem: in the scope of employee there is no attribute department.EmployeesOver(AvgSalary)\n#=>\nIt.department >>\nIt.employee >>\nFilter(It.salary .> mean.(It.department.employee.salary))\n=#This challenge can be overcome with Given, which evaluates its named arguments in the current scope and makes those values available within a subordinate pipeline.EmployeesOver(X) =\n  Given(:AMT => X,\n    It.department\n    >> It.employee\n    >> Filter(It.salary .> It.AMT))We could then combine these two pipelines.run(ChicagoData, EmployeesOver(AvgSalary))\n#=>\n  │ employee                            │\n  │ name       position          salary │\n──┼─────────────────────────────────────┼\n1 │ JEFFERY A  SERGEANT          101442 │\n2 │ DANIEL A   FIRE FIGHTER-EMT   95484 │\n=#Note that this expression is yet another pipeline that could be refined with further computation.run(ChicagoData,\n    EmployeesOver(AvgSalary)\n    >> It.name)\n#=>\n  │ name      │\n──┼───────────┼\n1 │ JEFFERY A │\n2 │ DANIEL A  │\n=#Although Given in this parameterized query defines It.amt, this computation\'s label doesn\'t leak outside the definition. run(ChicagoData,\n     EmployeesOver(AvgSalary)\n     >> It.amt)\n#-> ERROR: cannot find amt ⋮"
},
