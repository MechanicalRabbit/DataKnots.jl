...

{
    ...
    "title": "Cardinality",
    "category": "section",
    "text": "We have seen that queries produce any number of output rows: Lift(1:3) produces 3 rows and Lift(\"Hello World\") produces exactly one row. Further, the value missing, lifted to a constant query, never produces any rows.void[Lift(missing)]\n#=>\n│ It │\n┼────┼\n=#The constraint on the number of output rows a query may produce is called cardinality. A query is mandatory if its output must contain at least one row. It is singular if its output must contain at least one row.Example Singular Mandatory\nLift([]) No No\nLift(\'a\':\'c\') No No\nLift(missing) Yes No\nLift(\"Hello\") Yes YesIt\'s notable that queries producing vectors are never singular nor mandatory.void[Lift([\"One\"])]\n#=>\n  │ It  │\n──┼─────┼\n1 │ One │\n=#Finally, while the model permits mandatory and plural values, there isn\'t a Julia representation for this permutation."
},
