...

{
    ...
    "title": "Data shapes",
    "category": "section",
    "text": "In DataKnots, the structure of composite data is represented using shape objects.Consider, for example, a collection of departments with associated employees.depts =\n    @VectorTree (name = [String, REG],\n                 employee = [(name = [String, REG],\n                              position = [String, REG],\n                              salary = [Int, OPT],\n                              rate = [Float64, OPT]), PLU]) [\n        (name = \"POLICE\",\n         employee = [(name = \"JEFFERY A\", position = \"SERGEANT\", salary = 101442, rate = missing),\n                     (name = \"NANCY A\", position = \"POLICE OFFICER\", salary = 80016, rate = missing)]),\n        (name = \"FIRE\",\n         employee = [(name = \"JAMES A\", position = \"FIRE ENGINEER-EMT\", salary = 103350, rate = missing),\n                     (name = \"DANIEL A\", position = \"FIRE FIGHTER-EMT\", salary = 95484, rate = missing)]),\n        (name = \"OEMC\",\n         employee = [(name = \"LAKENYA A\", position = \"CROSSING GUARD\", salary = missing, rate = 17.68),\n                     (name = \"DORIS A\", position = \"CROSSING GUARD\", salary = missing, rate = 19.38)])\n    ]In this collection, each department record has two attributes: name and employee.  Each employee record has four attributes: name, position, salary, and rate.  The employee attribute is plural; salary and rate are optional.The structure of this collection can be described using NativeShape, OutputShape, and RecordShape objects.NativeShape specifies the type of atomic Julia data.NativeShape(String)\n#-> NativeShape(String)OutputShape specifies the label, the domain, and the cardinality of a record attribute.OutputShape(:position, NativeShape(String), REG)\n#-> OutputShape(:position, String)RecordShape describes the structure of a record.emp_shp =\n    RecordShape(OutputShape(:name, String),\n                OutputShape(:position, String),\n                OutputShape(:salary, Int, OPT),\n                OutputShape(:rate, Float64, OPT))Using nested shape objects, we can describe the structure of a nested collection.dept_shp =\n    RecordShape(OutputShape(:name, String),\n                OutputShape(:employee, emp_shp, PLU))"
},
...

{
    ...
    "category": "section",
    "text": "Among all queries, DataKnots distinguishes a special class of path-like queries.  They are called monadic.  We indicate that a query is monadic by assigning it its monadic signature.round_it = chain_of(lift(round), wrap())\n\nround_it = round_it |> designate(InputShape(Float64), OutputShape(Float64))\n\ndept_employee_round_rate = compose(dept_employee_rate, round_it)\n\ndept_employee_round_rate(depts)\n#-> @VectorTree [Float64] [[], [], [18.0, 19.0]]"
},
...

{
    ...
    "category": "section",
    "text": "Let\'s incrementally construct a set of patient records. Let\'s start with assigning a random 5-digit Medical Record Number (\"MRN\").RandPatient = Record(:mrn => Rand(10000:99999))\nrun(Several >> RandPatient)To assign an age to patients, we use Julia\'s truncated normal distribution. Since we wish whole-numbered ages, we truncate to the nearest integer value.RandPatient >>= Record(It.mrn,\n  :age => Trunc(Rand(TruncatedNormal(60,20,18,104))))\nrun(Several >> RandPatient)Let\'s assign each patient a random Sex. Here we use a categorical distribution plus enumerated values for male/female.@enum Sex male=1 female=2\nRandPatient >>= Record(It.mrn, It.age,\n  :sex => Lift(Sex, (Rand(Categorical([.492, .508])),)))\nrun(Several >> RandPatient)Next, let\'s define the patient\'s height based upon the U.S. average of 177cm for males and 163cm for females with distribution of 7cm.RandPatient >>= Record(It.mrn, It.age, It.sex,\n  :height => Trunc(Switch(It.sex, male => 177, female => 163)\n                   .+ Rand(TruncatedNormal(0,7,-40,40))))\nrun(Several >> RandPatient)"
},
