...

{
    ...
    "category": "section",
    "text": "Let\'s incrementally construct a set of patient records. Let\'s start with assigning a random 5-digit Medical Record Number (\"MRN\").RandPatient =\n   :patient => Record(:mrn => Rand(10000:99999))\nmake2to5(RandPatient)To assign an age to patients, we use Julia\'s truncated normal distribution. Since we wish whole-numbered ages, we truncate to the nearest integer value.RandPatient >>= Record(\n  :age => Trunc(Rand(TruncatedNormal(60,20,18,104))))\nmake2to5(RandPatient)Let\'s assign each patient a random Sex. Here we use a categorical distribution plus enumerated values for male/female.@enum Sex male=1 female=2\nRandPatient >>= Record(\n  :sex => Lift(Sex, (Rand(Categorical([.492, .508])),)))\nmake2to5(RandPatient)Next, let\'s define the patient\'s height based upon the U.S. average of 177cm for males and 163cm for females with distribution of 7cm.RandPatient >>= Record(\n  :height => Trunc(Switch(It.sex, male => 177, female => 163)\n                   .+ Rand(TruncatedNormal(0,7,-40,40))))\nmake2to5(RandPatient)"
},
