...

{
    ...
    "category": "section",
    "text": "Consider a pipeline Hello that produces a string value, \"Hello World\". It is built using the Const primitive, which converts a Julia string value into a pipeline component. This pipeline can then be run() to produce its output.Hello = Const(\"Hello World\")\nrun(Hello)\n#=>\n│ DataKnot    │\n├─────────────┤\n│ Hello World │\n=#The output of the pipeline is encapsulated in a DataKnot, which is a container holding structured, vectorized data. We can get the actual Julia string value using get().get(run(Hello)) #-> \"Hello World\"Consider another pipeline, Range(3). It is built with the Range combinator. When run(), it emits a sequence of integers from 1 to 3.run(Range(3))\n#=>\n  │ DataKnot │\n──┼──────────┤\n1 │        1 │\n2 │        2 │\n3 │        3 │\n=#The output of this knot can also be converted to native Julia.get(run(Range(3))) #-> [1, 2, 3]Observe that Hello pipeline produces a singular value, while the Range(3) pipeline is plural. In the output notation for plural knots, indices are in the first column with values in remaining columns."
},
