...

{
    ...
    "title": "Cardinality",
    "category": "section",
    "text": "When Julia values and functions are lifted, the type signature is inspected to discover how it should interact with query\'s flow. A flow is singular if it could have at most one element; else, it is plural. Furthermore, if a flow must have at least one element then it is mandatory; else, it is optional.Type Singular Mandatory\nVector{T} No No\nUnion{T, Missing} Yes No\n{T} Yes YesWhile the model permits mandatory and plural values, there isn\'t a Julia representation for this permutation."
},
...

{
    ...
    "title": "Paging Data",
    "category": "section",
    "text": "Like Filter, the Take and Drop combinators can be used to choose elements from an input: Drop is used to skip over input, while Take ignores input past a particular point.void[OneTo(9) >> Drop(3) >> Take(3)]\n#=>\n  │ It │\n──┼────┼\n1 │  4 │\n2 │  5 │\n3 │  6 │\n=#Unlike Filter, the argument to Take is evaluated once, in the context of the input\'s origin. To explain, let\'s first consider how a vector packed within a tuple could be listed.void[Lift((x=\'a\':\'c\', n=2)) >> It.x]\n#=>\n  │ x │\n──┼───┼\n1 │ a │\n2 │ b │\n3 │ c │\n=#Notice that n in this tuple is set to 2. It is a singular integer, so we can use it within the argument to Take.void[Lift((x=\'a\':\'c\', n=2)) >> Each(It.x >> Take(It.n))]\n#=>\n  │ x │\n──┼───┼\n1 │ a │\n2 │ b │\n=#In this next example, Take is performed three times. Each time, It refers to the integer elements of the outer loop, OneTo(3).void[OneTo(3) >> Each(Lift(\'a\':\'c\') >> Take(It))]\n#=>\n  │ It │\n──┼────┼\n1 │ a  │\n2 │ a  │\n3 │ b  │\n4 │ a  │\n5 │ b  │\n6 │ c  │\n=#More generally, how do we grab the 1st half of an input stream? Let\'s define FirstHalf as a combinator that builds a query returning the first half of an input stream.FirstHalf(X) = Each(X >> Take(Count(X) .÷ 2))\nvoid[FirstHalf(OneTo(6))]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  2 │\n3 │  3 │\n=#We could construct and register a FirstHalf query primitive.DataKnots.Lift(::typeof(FirstHalf)) = DataKnots.Then(FirstHalf)\n\nvoid[OneTo(6) >> FirstHalf]\n#=>\n  │ It │\n──┼────┼\n1 │  1 │\n2 │  2 │\n3 │  3 │\n=#"
},

{
    ...
    "category": "section",
    "text": "There is significant diversity how a query\'s input and output could be treated.Query Input Model Output Cardinality\nCount Aggregate Singular / Mandatory\nTake(N) Aggregate Plural   / Optional\nCount(X) Elementwise Singular / Mandatory\nFilter(P) Elementwise Singular / Optional\nLift(1:N) Elementwise Plural   / OptionalIn this processing model, query input and output are pipelines with two sides: an origin and a target, where each element in the origin is mapped to zero or more target elements.For any origin -> target pipeline, we can create two trivial pipelines, origin_pipe: origin -> origin, and target_pipe: target -> target. Therefore we have three options.Derivative Input Output\npass though Origin Target\ntarget_pipe Target Target\norigin_pipe Origin OriginA combinator has the option of what kind of input pipeline it could use for each of its arguments.Combinator Argument Input\nTake(N) N is an origin_pipe\nFilter(P) P is a target_pipe\nCount(X) X is a target_pipe\nLift(fn, (Xs...)) Xs... are target_pipeThe composition combinator (>>) with query arguments A and B, A >> B, deserves specific mention. The input of A is a pass though of the composition\'s input, while the input of B is the output of A.Also note that a combinator which builds only elementwise queries will necessarily use the target pipe to process its arguments."
},
