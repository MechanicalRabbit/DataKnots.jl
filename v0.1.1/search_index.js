...

{
    ...
    "title": "Constructors",
    "category": "section",
    "text": "A DataKnot object is created from a one-element cell vector and its shape.hello = DataKnot(ValueOf(String), [\"Hello World\"])\n#=>\n│ It          │\n┼─────────────┼\n│ Hello World │\n=#It is an error if the cell length is different from 1.DataKnot(ValueOf(String), String[])\n#-> ERROR: AssertionError: length(cell) == 1The cell and its shape can be retrieved.cell(hello)\n#-> [\"Hello World\"]\n\nshape(hello)\n#-> ValueOf(String)The shape could be specified by the element type.hello = DataKnot(String, [\"Hello World\"])\n\nshape(hello)\n#-> ValueOf(String)The shape could also be introspected from the given cell.hello = DataKnot(Any, [\"Hello World\"])\n\nshape(hello)\n#-> ValueOf(String)To make a DataKnot with a block cell, we can provide the block elements and its cardinality.abc = DataKnot(Any, \'a\':\'c\', :x1toN)\n#=>\n  │ It │\n──┼────┼\n1 │ a  │\n2 │ b  │\n3 │ c  │\n=#\n\ncell(abc)\n#-> @VectorTree (1:N) × Char [\'a\':\'\\x01\':\'c\']\n\nshape(abc)\n#-> BlockOf(Char, x1toN)Any Julia value can be converted to a DataKnot object using the convert() function.hello = convert(DataKnot, \"Hello World!\")\n#=>\n│ It           │\n┼──────────────┼\n│ Hello World! │\n=#Scalar values are stored as is.shape(hello)\n#-> ValueOf(String)The value missing is converted to an empty DataKnot.nullknot = convert(DataKnot, missing)\n#=>\n│ It │\n┼────┼\n=#\n\nshape(nullknot)\n#-> BlockOf(NoShape(), x0to1)The value nothing is converted to the unit DataKnot.  The unit DataKnot is exported under the name unitknot.unitknot\n#=>\n│ It │\n┼────┼\n│    │\n=#\n\nshape(unitknot)\n#-> ValueOf(Nothing)A vector value is converted to a block.vecknot = convert(DataKnot, \'a\':\'c\')\n#=>\n  │ It │\n──┼────┼\n1 │ a  │\n2 │ b  │\n3 │ c  │\n=#\n\nshape(vecknot)\n#-> BlockOf(Char)A Ref object is converted into the referenced value.int_ty = convert(DataKnot, Base.broadcastable(Int))\n#=>\n│ It  │\n┼─────┼\n│ Int │\n=#\n\nshape(int_ty)\n#-> ValueOf(Type{Int})"
},
