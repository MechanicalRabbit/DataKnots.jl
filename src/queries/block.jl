...
function flat_block(rt::Runtime, input::AbstractVector)
    @ensure_fits input BlockShape(BlockShape(AnyShape()))
    offs = offsets(input)
...
function pull_block(rt::Runtime, input::AbstractVector, lbl)
    @ensure_fits input TupleShape(lbl => BlockShape(AnyShape()), extra=true)
    j = locate(input, lbl)
    len = length(input)
    lbls = labels(input)
    cols = columns(input)
...
function any_block(rt::Runtime, input::AbstractVector)
    @ensure_fits input BlockShape(NativeShape(Bool))
    len = length(input)
    offs = offsets(input)
    elts = elements(input)
    if offs isa OneTo
