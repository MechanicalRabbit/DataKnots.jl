...
function correlate(rt::Runtime, input::AbstractVector)
    ...
    main isa SomeBlockVector || error("expected the first column to be a block vector; got $main\nat $(correlate())")
    mainoffs = offsets(main)
    mainelts = elements(main)
    mainelts isa SomeTupleVector && width(mainelts) == 2 || error("expected the first column to be a pair vector; got $mainelts\nat $(correlate())")
    dict isa SomeBlockVector || error("expected the second column to be a block vector; got $dict\nat $(correlate())")
    dictoffs = offsets(dict)
    dictelts = elements(dict)
    dictelts isa SomeTupleVector && width(dictelts) == 2 || error("expected the second column to be a pair vector; got $dictelts\nat $(correlate())")
